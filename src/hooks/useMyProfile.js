...
export default function StudentCalendar() {
  ...
  // ✅ lint(react-hooks/set-state-in-effect) 회피:
  // - effect 본문에서 "fetchWeek()" 직접 호출(즉시 setState 유발)하지 않고
  // - effect 내부 async IIFE에서 처리 + alive 가드로 레이스 방지
  useEffect(() => {
  ...
  // 이벤트 추가/삭제 후 재조회 함수 (effect 밖에서 호출용)
